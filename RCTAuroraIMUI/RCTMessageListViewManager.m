...
@interface RCTMessageListViewManager : RCTViewManager <IMUIMessageMessageCollectionViewDelegate,RCTMessageListDelegate>
...
- (void)messageCollectionViewWithDidMenuStatusViewInCell:(UICollectionViewCell * _Nonnull)didMenuStatusViewInCell model:(id <IMUIMessageModelProtocol> _Nonnull)model menuTitle:(NSString *)menuTitle;
...
RCT_EXPORT_VIEW_PROPERTY(onMenuItemClick, RCTBubblingEventBlock)
...
- (UIView *)view
{
  ...
  self.messageList = [[bundle loadNibNamed:@"RCTMessageListView" owner:self options: nil] objectAtIndex:0];
  self.messageList.messageList.delegate = self;
...
/// Tells the delegate that user tap statusView in message cell
- (void)messageCollectionViewWithDidTapStatusViewInCell:(IMUIBaseMessageCell * _Nonnull)didTapStatusViewInCell model:(id <IMUIMessageModelProtocol> _Nonnull)model {
...
- (void)messageCollectionViewWithDidMenuStatusViewInCell:(UICollectionViewCell * _Nonnull)didMenuStatusViewInCell model:(id <IMUIMessageModelProtocol> _Nonnull)model menuTitle:(NSString *)menuTitle{
    ...
    if(!_messageList.onMenuItemClick) { return; }

    RCTMessageModel *message = model;

    NSDictionary *messageDic = message.messageDictionary;

    NSString *msgType = ![model.messageType isEqualToString:@""] ? model.messageType : @"";

    NSMutableDictionary *mutableMessageDic = [NSMutableDictionary dictionaryWithDictionary:messageDic];

    [mutableMessageDic setObject:menuTitle forKey:@"MenuTitle"];
    [mutableMessageDic setObject:msgType forKey:@"msgType"];

//    NSLog(@"字典格式 = %@",mutableMessageDic);

    _messageList.onMenuItemClick((@{@"message": mutableMessageDic}));
